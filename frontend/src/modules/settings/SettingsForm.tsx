    ...
    <section className="rounded-lg border border-border bg-surface shadow-sm">
      <div className="flex flex-col gap-4 border-b border-border p-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          {description ? <p className="mt-1 text-sm text-text-body">{description}</p> : null}
        </div>
...
const inputClassName = (error?: boolean) =>
  [
    ...
    error ? "border-red-500 bg-red-50 text-red-900" : "border-border bg-surface text-text-heading",
  ].join(" ");
...
function FieldWrapper({ label, helperText, error, children }: FieldWrapperProps) {
  return (
    <label className="flex w-full flex-col gap-2">
      ...
      {children}
      {helperText ? (
        <span className={`text-xs ${error ? "text-red-600" : "text-text-body"}`}>{helperText}</span>
      ) : null}
    </label>
  );
}
...
function SwitchField({ label, checked, onChange }: SwitchFieldProps) {
  return (
    <label className="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-border bg-surface-subtle px-4 py-3 transition hover:bg-surface-alt">
      <span className="text-sm font-medium text-text-heading">{label}</span>
      <button
        type="button"
        role="switch"
      ...
      >
        <span
          className={`inline-block h-5 w-5 rounded-full bg-surface shadow transition ${
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        <span className="text-sm font-medium text-text-heading">{label}</span>
        {description ? <p className="text-xs text-text-body">{description}</p> : null}
      </div>
      ...
      {values.length > 0 ? (
        ...
        <p className="text-xs text-text-muted">Sin valores. Agrega alguno si aplica.</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-text-heading">Stakeholders y responsables</h3>
        <button
          type="button"
          className="rounded-md border border-indigo-500 px-3 py-2 text-sm font-medium text-indigo-600 transition hover:bg-indigo-50"
      ...
      {stakeholders.length === 0 ? (
        <p className="rounded-md border border-dashed border-border-strong bg-surface-subtle p-4 text-xs text-text-body">
...
function ChannelSelector({ value, onChange }: ChannelSelectorProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <span className="text-sm font-medium text-text-heading">Canales a monitorear</span>
    ...
    <div className="space-y-6">
      {INTEGRATION_GROUPS.map((group) => (
        <div key={group.title} className="space-y-3">
          <h3 className="text-sm font-semibold text-text-heading">{group.title}</h3>
          <div className="grid gap-3 md:grid-cols-2">
            {group.items.map((integration) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs text-text-body">{integration.hint}</span>
...
function ActionBar({ hasChanges, submitDisabled, status, onReset }: ActionBarProps) {
  return (
    <footer className="sticky bottom-0 flex flex-col gap-3 border-t border-border bg-surface/95 p-6 backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className="rounded-md border border-border-strong px-4 py-2 text-sm font-medium text-text-body transition hover:bg-surface-alt"
            disabled={!hasChanges}
            onClick={onReset}
          >
            Revertir cambios
...
function StatusBadge({ status }: StatusBadgeProps) {
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-full bg-surface-alt px-3 py-1 text-xs font-medium text-text-body">
