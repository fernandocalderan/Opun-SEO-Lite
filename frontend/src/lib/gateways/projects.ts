...
export async function createProject(payload: { name: string; primary_url: string; keywords?: string[]; monitoring_enabled?: boolean; schedule?: Project["schedule"] }) {
  ...
  const r = await fetch(`${API_BASE_URL}/v1/projects`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
...
export async function updateProject(id: string, patch: Partial<Omit<Project, "id">>) {
  if (!API_BASE_URL) return null;
  const r = await fetch(`${API_BASE_URL}/v1/projects/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) });
  if (!r.ok) throw new Error(String(r.status));
  return (await r.json()) as Project;
}
...
export async function deleteProject(id: string) {
  if (!API_BASE_URL) return null;
  const r = await fetch(`${API_BASE_URL}/v1/projects/${id}`, { method: "DELETE" });
  if (!r.ok) throw new Error(String(r.status));
  return true;
}
